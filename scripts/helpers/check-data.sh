...
if [[ ! -d "$raw_path" ]]; then
  echo "Warning: data/raw directory not found. Skipping integrity check."
  exit 0
fi
...
if [[ ! -f "$manifest_path" ]]; then
  echo "Warning: No manifest found in data/raw/. Run with --lock to secure data."
  exit 0
fi

echo "Verifying data integrity in data/raw/..."
