...
$rawPath = Join-Path $rootDir "data/raw"
$manifestPath = Join-Path $rawPath "manifest.sha256"

if (-not (Test-Path $rawPath)) {
    Write-Host "Warning: data/raw directory not found. Skipping integrity check." -ForegroundColor Gray
    exit 0
}
...
if (-not (Test-Path $manifestPath)) {
    Write-Host "Warning: No manifest found in data/raw/. Run with -Lock to secure data." -ForegroundColor Yellow
    exit 0
}

Write-Host "Verifying data integrity in data/raw/..." -ForegroundColor Cyan
